...
description: Audits one project's cartridge — its architecture, its own axiom department, its contract and its tooling. Use when a front closes inside that project having changed one of those. One auditor per project, never one across several.
---
...
⛔ **One auditor spread across eleven projects is one voice claiming eleven jurisdictions.**
...
data, an inbox being read (`skills/audit/`).
...
| # | Check |
|---|---|
| **P1** | **Does the project's own axiom department still hold?** The promotion review's *sideways* direction (`skills/company-auditor/`), on this department |
...
Per `skills/audit/`, with one difference: **it writes its ledger entry to that project's agent log**,
not the instance ledger — **a project auditor's standing is its own.**

## Dismissal

Standing and criterion per `skills/audit/`, **one of each per project**. ⚠️ **A finding repeated
across two projects is two findings** — the same defect in two places genuinely is, and one log
holding both would count it once.

**A project auditor that keeps agreeing is telling you the project is small enough not to need one**
— which is a useful finding and a cheap one, and the operator is the one who reads it.
