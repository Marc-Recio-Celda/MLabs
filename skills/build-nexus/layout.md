...
> three numbered roots**, and it names each **once**, so the gate prints three lines rather than one
> for every path.
>
...
⛔ **Every path in this file is relative to the operations centre's own root**, which is **a sibling
folder of the method repository** (`AGENTS.md` §2). A builder standing in the method repository
creates that folder beside it and works from inside it, so every `$SYS/…` and `$PROJ/…` below
resolves under `<the centre>/` and never under the method repository.
...
$PROJ/<project>/nexus/    —          that project's cartridge: every file structure-project writes
