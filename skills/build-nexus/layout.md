...
NB     = 93_Notebook  # the capture sheets. A ROOT of its own since 2026-09-05: the notebook
#                       became a project (`Notebook:D1`) and its sheets are not system files —
#                       they are the one place in the centre a human writes freely, and the one
#                       place its language rule bends
...
$SYS/WALL.md                   Compass    the task wall, seeded with *set your first task*
#                              ⚠️ DEBT 2026-09-05: this file still creates `$SYS/COMPASS.md`,
#                              `$SYS/PLAN.md` (live plan) and `$SYS/IDEAS.md` below. All three were
#                              retired by `M-134`/`M-135` — the wall replaced the compass, plans
#                              live in their projects, and the park is a notebook sheet. ⛔ A builder
#                              that builds the RETIRED shape is worse than no builder, because what
#                              it produces looks correct. Note `N-19`; `nexus:X8` owns it.
...
# ── the capture sheets ───────────────────────────────────────────────────────
$NB/README.md                  Standing   the capture contract: one line, four outcomes, the link
$NB/agents.md                  Queue      what an agent crossed (AX-25). Created empty
$NB/<owner>.md                 Queue      the operator's sheet, named by them. Created empty
$NB/ideas.md                   Park       deliberately not doing, with the reason
...
# ⚠️ FAST-TASKS.md is NOT created — retired 2026-09-05 into $NB/ (M-134).
# ── projects ─────────────────────────────────────────────────────────────────
