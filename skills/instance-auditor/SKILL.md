...
| **I6** | **`A6` on this instance's own standing files** — the binding, the instance department, `COMPANY.md`, `PURPOSE.md`. `skills/compact/` does the mechanical half; **this asks only whether a block that moved to a note left the operative layer incomplete.** ⚠️ **Never on the Records** — a log is supposed to accumulate. |
