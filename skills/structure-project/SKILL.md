...
### The file count was wrong three ways

This file said *"Six files, and no others"* over a table of **five** until 2026-08-21, while
`METHOD.md` §2 said **four plus two conditional** — **three different counts of the same thing, in
the skill that exists to make the count reproducible.** It also listed `architecture.md` as
unconditional while step 3 says it opens only once a rule exists, **so the table contradicted the
paragraph below it as well as the method.**

### Step 5 lost its heading

At some point before 2026-08-18 it lost its heading and half its text and sat as a dangling clause
mid-file. **Reconstructed, not recovered, and the file said so while the note existed** — the surviving
fragments were two axiom citations and two half-sentences, and **a reconstruction presented as the
original is the loss `PH-3` forbids.**

### Step 6b did not exist until 2026-08-19

**Its absence is exactly how a newly opened project and a newly introduced grouping folder both
reached one instance's tree with no denylist entry** — created the same week, invisible to a gate
that was otherwise working, and proven so against planted leaks while a control fired.

⚠️ **And until 2026-08-20 the step named a check that existed only as a shell function inside a
comment**, so it could be read and not run — **and on its first real run it found an uncovered
name.** A step naming an unrunnable check is the defect class this company has now found eleven
times.

### Prediction 1 was stale for a day

It said *"the six files of step 1"* after step 1 had been corrected to four-plus-two — **the fix
landed in the rule and not in the check that verifies it.** Found by the compaction pass.
